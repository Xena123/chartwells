...
|
| 1. Grunt Setup
|   a1. If If you have a package.json file from a project using
|       this setup, run following commands to install all
|       dependencies and skip remaining steps
npm outdated
npm i
|   a2. Create package.json file
npm init
...
|-------------------------------------------------------*/
  grunt.initConfig({
    ...
    watch: {
      ...
      html: {
        files: [ '<%= globalConfig.sitePath %><%= globalConfig.buildPath %>*.html' ],
        options : {
          spawn: false
        }
      },
    ...
    browserSync: {
      bsFiles: {
        src : ['<%= globalConfig.sitePath %><%= globalConfig.destCSS %>','*.html','**/*.php']
      },
      options: {
        watchTask: true,
        server: {
